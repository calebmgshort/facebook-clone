class User < ApplicationRecord
  ...
  def own_and_friends_posts
    ...
    Post.joins(:user).select("posts.id as id", :text, "users.id as user_id", "users.name as user_name", :created_at).order(created_at: :desc).where("users.id in ( #{friend_ids_string} )")  
  end

  def own_posts
    Post.joins(:user).select("posts.id as id", :text, "users.id as user_id", "users.name as user_name", :created_at).order(created_at: :desc).where(user_id: self.id)  
  end
